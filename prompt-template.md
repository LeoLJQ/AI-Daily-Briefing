...
**① 量子位 — RSS 优先（最可靠）：**
`curl -sL "https://www.qbitai.com/feed" | grep -oP '<pubDate>[^<]+</pubDate>'` 获取日期列表
从中筛选当天日期的文章，提取 `<title>` 和 `<link>`。

**② 机器之心 — 话题+日期搜索（官网链接不可用，须用转载链）：**
搜索："机器之心 jiqizhixin 2026年6月XX日 AI" + 当日热点关键词
⚠️ 机器之心官网（jiqizhixin.com）纯 JS 渲染，直接打开显示"数据服务"页面。
**必须找到文章在 163(m.163.com/dy/article/)、搜狐、36氪的转载链接作为最终 URL。**

**③ 极客公园 — 话题+日期搜索：**
搜索："极客公园 geekpark 2026年6月XX日" + 当日热点关键词
部分文章被新浪等平台转载，可通过转载链间接发现。

**④ 其余源 — 通用搜索：**
搜索："新智元/硅星人/DeepTech深科技/特工宇宙 + 当日日期 + 热点关键词"
...
**展示原则**：
- 搜索结果全量浏览，但只展示最重要的 8-15 篇
- 质量优先，不设同源数量上限
- **同一篇文章不重复**——如果同一篇文章（相同标题/相同链接）已经出现在新闻区或精选区，不要重复收录。不同来源对同一事件的不同角度报道不算重复，可以都收
- 优先选有独家视角、深度分析、产业洞察的文章
- 跳过纯新闻通稿、PR 稿、水文
- 每条附推荐理由，说明为什么这篇值得读
- 标注每篇文章来源，使用可打开的转载链接（非 JS 渲染官网）
